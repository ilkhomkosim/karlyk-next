...
const PropertyDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const likePropertyHandler = async (user: T, id: string) => {
		try {
			if (!id) return;
			if (!user._id) throw new Error(Message.NOT_AUTHENTICATED);
			//execution of likeTargetProperty Mutation
			await likeTargetProperty({ variables: { input: id } });

			await getPropertyRefetch({ input: id });

			// execution of getPropertiesRefetch
						...
						<Stack className={'property-info-config'}>
							...
							<Stack className={'images'}>
								<Stack className={'main-image'}>
									<img
										src={slideImage ? `${REACT_APP_API_URL}/${slideImage}` : '/img/property/homebgi.jpg'}
										alt={'main-image'}
									/>
								</Stack>
						...
						<Stack className={'property-desc-config'}>
							<Stack className={'left-config'}>
								<Stack className={'options-config'}>
									...
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										<img src={'/img/icons/leftcount.png'} />
										</Stack>
										<Stack className={'option-includes'}>
